...
export async function synchronizeSystemRbac(prisma: PrismaClient): Promise<SystemRbacSyncReport> {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const permissionsByCode = new Map(existingPermissions.map((permission) => [permission.code, permission]))
    const permissions = { created: 0, metadataUpdated: 0, unchanged: 0, removed: cleanup.permissionsRemoved }
    ...
    for (const definition of SYSTEM_PERMISSIONS) {
      ...
      else if (existing.name !== definition.name || existing.description !== definition.description)
    ...
    const groupsByName = new Map(existingGroups.map((group) => [group.name as SystemGroupName, group]))
    ...
    const permissionIdsByCode = new Map(syncedPermissions.map((permission) => [permission.code, permission.permissionId]))
    ...
    const assignmentKeys = new Set(existingAssignments.map((assignment) => `${assignment.groupId}:${assignment.permissionId}`))
    ...
    const created = missingAssignments.length
      ? (await tx.groupPermission.createMany({ data: missingAssignments, skipDuplicates: true })).count
      : 0

    return {
      permissions,
      groups,
      assignments: { desired, created, skipped: desired - created, removed: cleanup.assignmentsRemoved },
    }
...
async function removeObsoleteSystemPermissions(tx: Prisma.TransactionClient) {
  ...
  for (const permission of obsoletePermissions) {
    ...
    assignmentsRemoved += (await tx.groupPermission.deleteMany({
      where: {
    ...
    const remainingAssignments = await tx.groupPermission.count({ where: { permissionId: permission.permissionId } })
    if (remainingAssignments > 0) {
      throw new Error(`Cannot remove deprecated permission ${permission.code}; assignments changed during sync`)
    }
