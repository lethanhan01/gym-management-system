...
describe('synchronizeSystemRbac', () => {
  it('creates the catalog and mapping idempotently without delete operations', async () => {
    ...
    expect(first.permissions).toEqual({ created: 50, metadataUpdated: 0, unchanged: 0, removed: 0 })
    ...
    expect(second.permissions).toEqual({ created: 0, metadataUpdated: 0, unchanged: 50, removed: 0 })
    ...
    expect(memory.permissions.size).toBe(50)
  ...
  it('reconciles the audited legacy snapshot and removes only notification.send', async () => {
    ...
    expect(first.permissions).toEqual({ created: 13, metadataUpdated: 0, unchanged: 37, removed: 1 })
    expect(first.assignments).toEqual({ desired: 123, created: 36, skipped: 87, removed: 2 })
    expect(second.permissions).toEqual({ created: 0, metadataUpdated: 0, unchanged: 50, removed: 0 })
    expect(second.assignments).toEqual({ desired: 123, created: 0, skipped: 123, removed: 0 })
    expect(memory.permissions.has('notification.send')).toBe(false)
    expect(memory.permissions.size).toBe(50)
    expect(memory.assignments.size).toBe(123)
  })
