...
function setup({
...
}: { min?: number; transaction?: jest.Mock; client?: ExerciseDbV2Client } = {}) {
  const tx: TransactionMock = {
    ...
    exerciseCatalogSyncRun: {
      update: jest.fn().mockResolvedValue({ syncRunId: 1n, status: 'succeeded' }),
    },
    $executeRaw: jest.fn(),
  }
  const prisma: PrismaMock = {
    ...
    exerciseCatalogSyncRun: {
      create: jest.fn().mockResolvedValue({ syncRunId: 1n }),
      update: jest.fn(),
    },
  ...
  const config = {
    get: jest.fn(
      (key: string) =>
        ({
          EXERCISEDB_LOCK_LEASE_SECONDS: 300,
          EXERCISEDB_MIN_EXPECTED_COUNT: min,
          EXERCISEDB_UPSERT_BATCH_SIZE: 1,
        })[key]
    ),
  }
  return {
...
describe('ExerciseCatalogSyncService', () => {
  it('rejects a provider payload below the configured minimum before starting a database transaction', async () => {
    ...
    expect(prisma.exerciseCatalogSyncRun.update).toHaveBeenCalledWith(
  ...
  it('commits all catalog mutations through one transaction and uses one SQL upsert per configured batch', async () => {
    ...
    expect(tx.exerciseCatalogSyncRun.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: 'succeeded', insertedCount: 1 }),
      })
    )
  })
  ...
  it('refuses a manifest mismatch before starting a transaction or transitioning legacy exercises', async () => {
    ...
    const manifest = {
      ...
      requestCount: 1,
      firstExternalId: 'push-up',
      lastExternalId: 'push-up',
  ...
  it('accepts a matching manifest and keeps legacy transition inside the commit transaction', async () => {
    ...
    const manifest = {
      ...
      pageSize: 10 as const,
      count: 1,
      externalIdHash: externalIdHash([item]),
      requestCount: 1,
      firstExternalId: 'push-up',
      lastExternalId: 'push-up',
      generatedAt: new Date().toISOString(),
  ...
  it('counts matching existing rows as unchanged while still issuing one batch upsert', async () => {
    ...
    tx.exercise.findMany.mockResolvedValue([
      { externalId: 'push-up', contentHash: item.contentHash, exerciseId: 1n },
    ])
    ...
    expect(tx.exerciseCatalogSyncRun.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ insertedCount: 0, updatedCount: 0, unchangedCount: 1 }),
      })
    )
    expect(tx.$executeRaw).toHaveBeenCalledTimes(1)
  })
