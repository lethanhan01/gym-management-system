...
export class ExerciseCatalogController {
  ...
  async run() { return { success: true, ...(await this.sync.run()) } }

  @Get('sync-runs')
  @RequirePermission('exercise.sync')
  async runs(@Query('limit') limit?: string) { return { success: true, data: await this.sync.recentRuns(limit ? Number(limit) : undefined) } }
}
