...
export class MaintenanceService {
  ...
  async listMaintenanceLogs(equipmentId: bigint, query: ListMaintenanceLogsDto) {
    ...
    return {
      data: data.map((row) => this.serializeMaintenanceLog(row)),
  ...
  async createMaintenanceLog(
    ...
    if (equipment.status === EquipmentStatus.retired) {
      throw new ConflictException({
        ...
        message: 'Thiết bị đã thanh lý, không thể tạo maintenance log mới',
      })
    }
    ...
    if (openCount > 0) {
      throw new ConflictException({
        ...
        message: 'Thiết bị đã có maintenance mở, không thể báo thêm',
      })
    }

    const reporterStaff = await this.prisma.staff.findFirst({
    ...
    if (!reporterStaff) {
      throw new ForbiddenException({
        success: false,
        code: 'FORBIDDEN',
        message: 'Người báo hỏng phải có hồ sơ staff',
      })
    }
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (equipment.status === EquipmentStatus.active) {
        equipmentUpdated = await tx.equipment.update({
          where: { equipmentId },
    ...
    this.audit.log({
      ...
      afterData: {
        ...
        equipment_status_change:
          equipment.status === EquipmentStatus.active ? { from: 'active', to: 'broken' } : null,
    ...
    return {
      data: {
        ...
        equipment: {
          equipmentId: result.equipmentUpdated.equipmentId.toString(),
          equipmentCode: result.equipmentUpdated.equipmentCode,
          status: result.equipmentUpdated.status,
        },
      },
    }
  }
  ...
  async updateMaintenanceLog(
    ...
    const existing = await this.prisma.maintenanceLog.findFirst({
      ...
      include: {
        ...
        reportedByStaff: { include: { user: true } },
      },
    })
    if (!existing)
      throw new NotFoundException({
        success: false,
        code: 'NOT_FOUND',
        message: 'Maintenance log không tồn tại',
      })

    if (
      existing.status === MaintenanceStatus.resolved ||
      existing.status === MaintenanceStatus.failed
    ) {
      throw new ConflictException({
        success: false,
        code: 'MAINTENANCE_ALREADY_CLOSED',
        message: 'Maintenance log đã đóng, không thể cập nhật tiếp',
      })
    }
    ...
    if (
      ...
      existing.status !== MaintenanceStatus.reported
    ) {
      throw new ConflictException({
        success: false,
        code: 'MAINTENANCE_INVALID_STATE_TRANSITION',
        message: 'Chỉ có thể chuyển reported -> repairing',
      })
    }
    if (
      (nextStatus === MaintenanceStatus.resolved || nextStatus === MaintenanceStatus.failed) &&
      existing.status !== MaintenanceStatus.repairing
    ) {
      throw new ConflictException({
        success: false,
        code: 'MAINTENANCE_INVALID_STATE_TRANSITION',
        message: 'Chỉ có thể chuyển repairing -> resolved/failed',
      })
    }
    ...
    this.audit.log({
      actorUserId,
      action:
        nextStatus === MaintenanceStatus.repairing ? 'maintenance.update' : 'maintenance.resolve',
      resourceType: 'maintenance_log',
      resourceId: maintenanceId.toString(),
      beforeData: this.serializeMaintenanceLog(existing) as unknown as Record<string, unknown>,
      afterData: {
        ...this.serializeMaintenanceLog(result.log),
        equipment_status: result.equipment.status,
      } as unknown as Record<string, unknown>,
    })
    ...
    return {
      data: {
        maintenance: this.serializeMaintenanceLog(result.log),
        equipment: {
          equipmentId: result.equipment.equipmentId.toString(),
  ...
  private serializeMaintenanceLog(log: {
    maintenanceId: bigint
    equipmentId: bigint
    ...
    return {
      ...
      reportedByStaff: log.reportedByStaff
        ? {
            staffId: log.reportedByStaff.staffId.toString(),
            staffCode: log.reportedByStaff.staffCode,
            fullName: log.reportedByStaff.user.fullName,
          }
        : { staffId: log.reportedByStaffId.toString() },
