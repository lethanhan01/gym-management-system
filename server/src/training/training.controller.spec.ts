...
describe('TrainingController', () => {
  ...
  describe('attendance routes', () => {
    ...
    it('returns QR token generated by the service', async () => {
      const token = {
        token: 'v1.2026-07-22.signature',
        payload: { version: 'v1', date: '2026-07-22' },
        validDate: '2026-07-22',
        expiresAt: '2026-07-22T16:59:59.999Z',
      }
      ;(mockService.generateQrToken as jest.Mock).mockReturnValue(token)

      const res = await ctrl.getQrToken()

      expect(mockService.generateQrToken).toHaveBeenCalled()
      expect(res).toEqual({ success: true, data: token })
    })

    it('delegates QR check-in with member caller context', async () => {
      const dto = { token: 'v1.2026-07-22.signature' } as any
      const serviceResult = { data: { attendanceId: '12' } }
      ;(mockService.qrCheckin as jest.Mock).mockResolvedValue(serviceResult)

      const res = await ctrl.qrCheckin(dto, memberUser)

      expect(mockService.qrCheckin).toHaveBeenCalledWith(dto, {
