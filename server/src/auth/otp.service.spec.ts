...
describe('OtpService', () => {
  ...
  it('persists a replacement OTP and throttle record', async () => {
    ...
    expect(tx.otpCode.upsert).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('does not issue another OTP after the persistent rate limit', async () => {
    tx.otpRequestThrottle.findUnique.mockResolvedValue({
      windowStartedAt: new Date(), requestCount: 3,
    })
  ...
  it('consumes a valid OTP exactly once', async () => {
    ...
    prisma.otpCode.findUnique.mockResolvedValue({ otpCodeId: 7n, expiresAt: new Date(Date.now() + 60_000), attemptCount: 0, codeHash: hash })
    ...
    expect(prisma.otpCode.deleteMany).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ otpCodeId: 7n }) }))
  })
})
