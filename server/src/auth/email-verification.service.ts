  ...
  async verifyEmail(email: string, code: string, ctx: RequestContext = {}): Promise<void> {
    ...
    if (result === 'invalid') {
      await this.audit.log({
        ...
        afterData: { success: false, reason: 'invalid_otp' },
        ipAddress: ctx.ip,
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (isDemoOtp(code))
        await tx.otpCode.deleteMany({
          where: { userId: user.userId, purpose: OtpPurpose.email_verify },
        })
    })
    await this.audit.log({
      actorUserId: user.userId,
  ...
  async resendVerify(email: string, ctx: RequestContext = {}): Promise<{ message: string }> {
    ...
    try {
      ...
      await this.audit.log({
        ...
        resourceType: 'auth',
        resourceId: user.userId.toString(),
        afterData: { step: 'resend', delivery_failed: true },
        ipAddress: ctx.ip,
        userAgent: ctx.userAgent,
      })
      return { message }
