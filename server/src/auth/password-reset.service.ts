...
export class PasswordResetService {
  ...
  async forgotPassword(email: string, ctx: RequestContext = {}): Promise<{ message: string }> {
    ...
    try {
      ...
      await this.audit.log({
        ...
        afterData: { step: 'request', delivery_failed: true },
        ipAddress: ctx.ip,
  ...
  async verifyResetOtp(email: string, code: string, ctx: RequestContext = {}): Promise<string> {
    ...
    const result = isDemoOtp(code)
      ? 'valid'
      : await this.otp.verify(user.userId, OtpPurpose.password_reset, code)
    ...
    await this.prisma.passwordResetGrant.upsert({
      ...
      create: {
        userId: user.userId,
        tokenHash: this.hashGrant(grant),
        expiresAt: new Date(Date.now() + this.grantTtlMs),
      },
      update: {
        tokenHash: this.hashGrant(grant),
        expiresAt: new Date(Date.now() + this.grantTtlMs),
    ...
    await this.audit.log({
      ...
      resourceType: 'auth',
      resourceId: user.userId.toString(),
      afterData: { step: 'verify_otp', success: true },
      ipAddress: ctx.ip,
      userAgent: ctx.userAgent,
    })
    return grant
  ...
  async resetPassword(
    ...
    if (!grant || grant.expiresAt <= new Date()) {
      if (grant)
        await this.prisma.passwordResetGrant.deleteMany({
          where: { passwordResetGrantId: grant.passwordResetGrantId },
        })
    ...
    const consumed = await this.prisma.$transaction(async (tx) => {
      const result = await tx.passwordResetGrant.deleteMany({
        where: {
          passwordResetGrantId: grant.passwordResetGrantId,
          tokenHash,
    ...
    await this.audit.log({
      ...
      action: 'auth.password-reset',
      resourceType: 'auth',
      resourceId: grant.userId.toString(),
