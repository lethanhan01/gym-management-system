  ...
  return (
    <MemberPage>
      ...
      {loading ? (
        ...
        <MemberErrorState message={profileError} onRetry={() => {
          setLoading(true)
          setProfileError(null)
          memberService.getProfile(user?.memberId ?? '')
            .then((data) => setProfile(data))
            .catch((err) => setProfileError(getApiError(err, t('profile.errorLoad'))))
            .finally(() => setLoading(false))
        }} />
        ...
        <main className="grid gap-5 xl:grid-cols-2">
          <Card as="article" variant="compact" className="p-6 flex flex-col">
            <header className="mb-5 flex items-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl [background:color-mix(in_srgb,var(--rogym-teal)_12%,transparent)] rogym-text-accent">
                <UserRound size={22} />
              </div>
            ...
            <footer className="mt-auto pt-6 flex gap-3">
              {isEditing ? (
                <>
                  <Button
          ...
          {/* LINE Account Link */}
          <Card as="article" variant="compact" className="p-6 xl:col-span-2">
            <header className="mb-4 flex items-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl [background:color-mix(in_srgb,var(--rogym-green)_12%,transparent)]">
            ...
            {lineError && (
              <Alert tone="error" description={lineError} className="mb-3" />
            )}
