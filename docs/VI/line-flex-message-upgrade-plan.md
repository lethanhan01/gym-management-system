...
> ✅ **Trạng thái**: **ĐÃ HOÀN THÀNH 100% (ALL 5 PHASES COMPLETED & RATIFIED)**

---

## 1. Mục Tiêu & Nguyên Tắc Triển Khai

1. **Nâng tầm trải nghiệm người dùng**: Chuyển đổi toàn bộ thông báo LINE từ dạng văn bản đơn giản (Plain Text + Quick Reply) sang **LINE Flex Message (Bubble Card)** trực quan, có nhận diện thương hiệu RoGym Dark Theme, màu sắc trạng thái rõ ràng và nút thao tác sâu (LIFF Deep-link).
...
> - 📄 [**Phase 1: Core Flex Builder & Unit Tests**](./line-flex-upgrade/phase-1-core-flex-builder.md) (✅ Completed)
> - 📄 [**Phase 2: Service Refactoring & Graceful Fallback**](./line-flex-upgrade/phase-2-service-refactoring.md) (✅ Completed)
> - 📄 [**Phase 3: Bổ Sung 3 Sự Kiện Mới (Payment, Completed, Feedback)**](./line-flex-upgrade/phase-3-missing-events-integration.md) (✅ Completed)
> - 📄 [**Phase 4: Dev Mock Sandbox & Giao Diện Preview**](./line-flex-upgrade/phase-4-dev-mock-and-ui.md) (✅ Completed)
> - 📄 [**Phase 5: Kiểm Thử Toàn Diện E2E & Hoàn Thiện Tài Liệu**](./line-flex-upgrade/phase-5-e2e-and-verification.md) (✅ Completed)
...
- [x] **Task 1.1**: Tạo file `server/src/line-messaging/line-flex-builder.ts`:
...
- [x] **Task 1.2**: Tạo file `server/src/line-messaging/line-flex-builder.spec.ts`:
...
- [x] `line-flex-builder.ts` xuất bản đầy đủ 12 hàm builder theo chuẩn LINE Flex Message v2.
- [x] Lệnh `npm run test -- server/src/line-messaging/line-flex-builder.spec.ts` pass 100% không có cảnh báo.
...
#### 1. Các công việc cần làm:
- [x] **Task 2.1**: Mở rộng type `LineMessage` trong `line-messaging.service.ts` để hỗ trợ cả `type: 'flex'` (với `altText` và `contents: LineFlexContainer`) và `type: 'text'`.
- [x] **Task 2.2**: Cập nhật các hàm push & reply hiện tại sang dùng `LineFlexBuilder`:
...
- [x] **Task 2.3**: Xây dựng cơ chế **Graceful Fallback**:
...
- [x] **Task 2.4**: Cập nhật `server/src/line-messaging/line-messaging.service.spec.ts`:
...
- [x] 7 sự kiện hiện tại và 2 webhook actions gửi đúng định dạng Flex Message.
- [x] Nhánh Fallback hoạt động tin cậy khi có dữ liệu lỗi.
- [x] Lệnh `npm run test -- server/src/line-messaging/line-messaging.service.spec.ts` pass 100%.
...
#### 1. Các công việc cần làm:
- [x] **Task 3.1**: Bổ sung các phương thức push mới vào `LineMessagingService`:
...
- [x] **Task 3.2**: Tích hợp gọi `safePush...` vào các nghiệp vụ tương ứng:
...
- [x] **Task 3.3**: Cập nhật unit tests cho:
...
- [x] Khi kích hoạt thanh toán thành công, hoàn thành buổi tập hoặc phản hồi feedback, nếu user có `lineId`, hệ thống gửi đúng Flex Card tương ứng.
- [x] Toàn bộ test suite của Payments, Feedback, Training pass 100%.
...
#### 1. Các công việc cần làm:
- [x] **Task 4.1**: Cập nhật `server/src/line-messaging/line-mock.controller.ts`:
  - Mở rộng enum `LineMockSample` hỗ trợ đầy đủ 14 loại sample.
  - Cập nhật hàm `createMockSample` trong `LineMessagingService` tạo payload mẫu đầy đủ cho tất cả các loại trên.
- [x] **Task 4.2**: Cập nhật `client/src/pages/dev/LineMockInboxPage.tsx`:
  - Bổ sung nhóm nút bấm Quick Samples phân loại theo 4 bảng Box Card (Lịch tập PT, Điểm danh, Gói & Thanh toán, Phản hồi, Webhook).
  - Tinh chỉnh component `FlexPreview` để render chuẩn các thuộc tính Flex Message mới (Badge màu sắc, separator, key-value 2 cột, dual buttons, Dark Theme RoGym `#0f1c16`).
  - Hỗ trợ click Action Buttons trên thẻ Flex để nạp ngay vào Mobile Phone Simulator bên phải.
- [x] **Task 4.3**: Cập nhật `client/src/pages/dev/LineMockInboxPage.test.tsx` kiểm thử việc bấm các nút sample mới và click CTA.
...
- [x] Trên trang `/dev/line-mock`, có đầy đủ các nút bấm tạo mẫu cho tất cả các sự kiện.
- [x] Giao diện giả lập điện thoại LINE hiển thị thẻ Flex Card đẹp mắt, đúng định dạng màu sắc và font chữ.
- [x] `npm run test -- client/src/pages/dev/LineMockInboxPage.test.tsx` pass 100%.
...
#### 1. Các công việc cần làm:
- [x] **Task 5.1**: Chạy toàn bộ test suite của Server và Client:
  - `npm test` trên `server/` (Toàn bộ 75 test suites, 1107 tests PASS).
  - `npm test` trên `client/` (Toàn bộ 70 test files, 326 tests PASS).
- [x] **Task 5.2**: Xây dựng bộ kiểm thử tích hợp End-to-End (`server/src/line-messaging/line-flex-e2e.spec.ts`):
  - Kịch bản 1: Hội viên đặt lịch tập PT $\rightarrow$ Flex Card Tone success + CTA deep-link.
  - Kịch bản 2: PT đổi lịch / hủy lịch $\rightarrow$ Flex Card Tone info / Tone danger.
  - Kịch bản 3: Điểm danh QR tại phòng tập $\rightarrow$ Flex Card check-in.
  - Kịch bản 4: Thanh toán thành công $\rightarrow$ Flex Card biên lai thanh toán.
  - Kịch bản 5: Chuyển đổi ngôn ngữ hệ thống sang Tiếng Nhật (`LINE_MESSAGE_LOCALE=ja`) $\rightarrow$ Toàn bộ thẻ hiển thị chuẩn tiếng Nhật và tiền tệ Yên.
  - Kịch bản 6: Kiểm thử cơ chế Graceful Fallback về Plain Text khi builder gặp ngoại lệ.
  - Kịch bản 7: Kiểm thử toàn diện 14 sample triggers qua Mock Controller.
- [x] **Task 5.3**: Cập nhật tài liệu kỹ thuật [`docs/VI/line-messaging-specification.md`](file:///c:/Users/An/Documents/IT4549-ITSS/gym-management-system/docs/VI/line-messaging-specification.md):
...
1. [x] **Chất lượng Code**: 100% tests backend và frontend pass, không có bất kỳ regression bug nào.
2. [x] **Trực quan & Thẩm mỹ**: 100% thông báo LINE hiển thị dưới dạng Flex Message Bubble Card đồng nhất theo nhận diện RoGym.
3. [x] **Độ tin cậy (Resilience)**: Hệ thống hoạt động ổn định kể cả khi gặp lỗi kết nối LINE API hoặc dữ liệu thiếu sót (nhờ Graceful Fallback).
4. [x] **Tài liệu**: Tài liệu kỹ thuật `docs/VI/line-messaging-specification.md` và `docs/VI/README.md` được đồng bộ chính xác.
